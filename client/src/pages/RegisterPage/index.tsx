...
export function RegisterPage() {
  ...
  const validatePassword = (value: string) => {
    ...
    if (!value) {
      ...
      error = '비밀번호는 문자와 숫자를 포함해야 합니다';
    }
    return error;
  };

  const validatePasswordConfirm = (value: string, passwordValue: string) => {
    let error = '';
    if (!value) {
...
<input
  ...
  onChange={(e) => {
    ...
    const error = validatePasswordConfirm(value, password);
    setValidationErrors(prev => ({ ...prev, passwordConfirm: error }));
  }}
  className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
  placeholder="비밀번호를 다시 입력하세요"
  required
/>
{touched.passwordConfirm && validationErrors.passwordConfirm && (
  <div className="text-red-400 text-sm mt-1">
